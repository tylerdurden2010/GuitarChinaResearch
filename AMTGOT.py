...
prs = re.compile(r'amt|m2',re.I)
resultfilter = re.compile(r'(?P<tag1>\<em\>.*\<\/em\>\s.*\<span id=".*"\>\<a href\=\")(?P<tag2>thread\-\d.*.html)\"\>(?P<tag3>.*\<\/a>)')
file = open("c:\\resultamt.txt",'ab')
#f = """GET /forum-100-5.html HTTP/1.1http://bbs.guitarchina.com/forum-102-1.html
for i in range(1,150):
    req = urllib2.Request("http://bbs.guitarchina.com/forum-102-"+str(i)+".html")
